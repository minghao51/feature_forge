...
def _load_semantic_package(
    ...
    """Verify package integrity and the exact manifest-only upstream chain.

    Layer-specific semantic loaders are added with their dataflow modules. The
    durable foundation still enforces the authoritative manifest edge here.
    """
    manifest = store.load_manifest(ref)
    expected: list[ManifestRef] = []
    if ref.layer is Layer.SILVER:
        if (bronze := chain.get(Layer.BRONZE)) is None:
            raise ValueError("Silver package is missing its planned Bronze parent")
        expected = [bronze]
    elif ref.layer is Layer.GOLD:
        if (silver := chain.get(Layer.SILVER)) is None:
            raise ValueError("Gold package is missing its planned Silver parent")
        expected = [silver]
    elif ref.layer is Layer.PLATINUM:
        if (silver := chain.get(Layer.SILVER)) is None or (gold := chain.get(Layer.GOLD)) is None:
            raise ValueError("Platinum package is missing its planned Silver/Gold parents")
        expected = [silver, gold]
    if manifest.upstream_manifests != expected:
        raise ValueError(f"{ref.layer.value} package does not reference the exact planned chain")
