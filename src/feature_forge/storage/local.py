...
def _scan_mtime_ns(path: Path) -> int:
    """Best-effort mtime for reuse ordering; unreadable candidates sort last."""
    try:
...
class LocalArtifactStore:
    ...
    def find_reusable(
        ...
        *,
        ...
        upstreams: Sequence[ManifestRef] | None = None,
        preferred_run_id: str | None = None,
        allow_cross_run: bool = True,
    ) -> ManifestRef | None:
        """Find a verified package by reuse fingerprint and exact lineage.

        ``upstream`` is retained for callers that have a single parent. New
        callers should pass the complete ordered ``upstreams`` sequence.

        Only hash- and manifest-verified packages can be returned: every
        candidate is verified before its fingerprint or lineage is consulted.
        Invalid cross-run candidates are skipped so policy may recompute the
        layer, while an invalid preferred package fails closed with an error.
        """
        if upstream is not None and upstreams is not None:
            raise ValueError("pass either upstream or upstreams, not both")
        expected_upstreams = (
            list(upstreams)
            if upstreams is not None
            else ([upstream] if upstream is not None else [])
        )
        ...
        for package in candidates:
            ...
            try:
                ...
                continue
            reusable_identity = manifest.reuse_fingerprint or manifest.layer_fingerprint
            if reusable_identity != fingerprint_value:
                continue
