...
class StageExecution(ContractModel):
    """Verified contribution of one durable layer to a case attempt."""

    schema_version: Literal["1"] = "1"
    layer: Layer
    disposition: StageDisposition
    manifest_ref: ManifestRef
    reuse_fingerprint: str = Field(min_length=1)
    layer_fingerprint: str = Field(min_length=1)


class CaseExecutionPlan(ContractModel):
    """Read-only static plan whose source-dependent layers remain explicit."""
    ...
    unresolved_layers: list[Layer] = Field(default_factory=lambda: list(Layer))
