...
class TestTrackerContract:
    def test_wandb_implements_tracker(self) -> None:
        assert issubclass(WandBTracker, ExperimentTracker)

    def test_mlflow_implements_tracker(self) -> None:
        assert issubclass(MLflowTracker, ExperimentTracker)

    def test_noop_implements_tracker(self) -> None:
        assert issubclass(NoOpTracker, ExperimentTracker)

    def test_tracker_has_required_methods(self) -> None:
...
class TestWandBTrackerInit:
    ...
    def test_init_custom_flags(self) -> None:
    ...
    def test_init_run_no_wandb_raises(self) -> None:
        tracker = WandBTracker(project="test")
        with patch.dict("sys.modules", {"wandb": None}):
            with pytest.raises(TrackingError, match="wandb not installed"):
                tracker.init_run("run1", {"x": 1})

    def test_log_metrics_no_run_no_error(self) -> None:
        tracker = WandBTracker(project="test")
        tracker.log_metrics({"acc": 0.9})

    def test_log_params_no_run_no_error(self) -> None:
        tracker = WandBTracker(project="test")
        tracker.log_params({"lr": 0.01})

    def test_log_artifact_no_run_no_error(self) -> None:
        tracker = WandBTracker(project="test")
        tracker.log_artifact("path/to/file")

    def test_finish_no_run_no_error(self) -> None:
        tracker = WandBTracker(project="test")
        tracker.finish()
...
class TestMLflowTrackerInit:
    ...
    def test_init_with_tracking_uri(self) -> None:
        tracker = MLflowTracker(project="test", tracking_uri="http://localhost:5000")
        assert tracker.tracking_uri == "http://localhost:5000"

    def test_init_run_no_mlflow_raises(self) -> None:
        tracker = MLflowTracker(project="test")
        with patch.dict("sys.modules", {"mlflow": None}):
            with pytest.raises(TrackingError, match="mlflow not installed"):
                tracker.init_run("run1", {"x": 1})

    def test_log_metrics_no_mlflow_no_error(self) -> None:
        tracker = MLflowTracker(project="test")
        with patch.dict("sys.modules", {"mlflow": None}):
            tracker.log_metrics({"acc": 0.9})

    def test_log_params_no_mlflow_no_error(self) -> None:
        tracker = MLflowTracker(project="test")
        with patch.dict("sys.modules", {"mlflow": None}):
            tracker.log_params({"lr": 0.01})

    def test_log_artifact_no_mlflow_no_error(self) -> None:
        tracker = MLflowTracker(project="test")
        with patch.dict("sys.modules", {"mlflow": None}):
            tracker.log_artifact("path/to/file")

    def test_finish_no_mlflow_no_error(self) -> None:
        tracker = MLflowTracker(project="test")
        with patch.dict("sys.modules", {"mlflow": None}):
            tracker.finish()
...
class TestLogArtifactItem:
    ...
    def test_none_logs_as_param(self) -> None:
    ...
    def test_code_string_logs_as_code(self) -> None:
        tracker = NoOpTracker(project="test")
    ...
    def test_dict_logs_as_json_param(self) -> None:
        tracker = NoOpTracker(project="test")
        with patch.object(tracker, "log_params") as mock_log:
    ...
    def test_lazy_ref_load_fails(self) -> None:
        tracker = NoOpTracker(project="test")
...
class TestCreateTrackerFromConfig:
    ...
    def test_none_backend_returns_noop(self) -> None:
        from feature_forge.config import TrackerConfig
    ...
    def test_returns_experiment_tracker(self) -> None:
        from feature_forge.config import TrackerConfig
