...
class TestSandboxMarkdownFenceHandling:
    def test_code_with_python_fence_succeeds_after_strip(self) -> None:
...
```"""
    ...
    def test_code_with_python_fence_fails_without_strip(self) -> None:
        executor = SandboxedExecutor(timeout_seconds=5.0)
        raw_llm_output = """```python
import pandas as pd
...
```"""
    ...
    def test_code_with_bare_fence_succeeds_after_strip(self) -> None:
        executor = SandboxedExecutor(timeout_seconds=5.0)
        raw_llm_output = """```
import pandas as pd
...
```"""
    ...
    def test_plain_code_works_without_strip(self) -> None:
        executor = SandboxedExecutor(timeout_seconds=5.0)
