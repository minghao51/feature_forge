...
class TestNoOpTracker:
    ...
    def test_log_artifacts_dict_with_values(self) -> None:
        import pandas as pd
    ...
    def test_log_artifacts_dict_with_prefix(self) -> None:
        tracker = NoOpTracker(project="test-prefix")
        tracker.log_artifacts_dict({"val": 1.0}, prefix="round_0_")

    def test_log_artifacts_dict_with_lazy_ref(self) -> None:
    ...
    def test_log_artifacts_dict_with_none(self) -> None:
...
class TestReporter:
    ...
    def test_get_best_empty_results(self) -> None:
        reporter = Reporter([])
        best = reporter.get_best(metric="score", group_by="dataset")
        assert len(best) == 0
